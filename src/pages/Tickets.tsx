...
import { ShieldAlert, CheckCircle2, AlertCircle, Clock, Search, Ticket } from 'lucide-react';
  ...
  return (
    <div className="space-y-8 relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div>
          <h1 className="text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-white to-slate-400 tracking-tight">Tickets</h1>
          <p className="text-slate-400 mt-2 text-lg">Manage and resolve reported student tickets.</p>
        </div>
      </div>
              ...
              <div className="flex items-start gap-4 flex-1 min-w-0">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-3 mb-1">
                    <h3 className="text-lg font-bold text-white truncate">PC {issue.pc_id}</h3>
                    <span className={cn(
                  ...
                  <p className="text-slate-300 text-sm mb-2 break-words">{issue.description}</p>
                  <div className="flex items-center gap-2 text-xs text-slate-500 font-medium">
                    <Clock className="w-3.5 h-3.5 shrink-0" />
                    <span className="truncate">Reported {formatDistanceToNow(new Date(issue.created_at), { addSuffix: true })}</span>
                  </div>
                </div>
              </div>
